...
<script>
	...
	function send(){
		...
		alert("submit");
		frm.action="User?cmd=user_modify_pro";
		frm.submit();
	}
					...
					<td bgcolor="#7AAAD5">            
						...
						<table cellpadding=3 cellspacing=1 border=0 width=100%>
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<input type=text name=name size=16 maxlength=20 value="${Session_User.name}" readonly>성명은 빈칸없이 입력하세요.
								</td>
							</tr>
							<tr>
								...
								<TD BGCOLOR=WHITE>
									<table cellspacing=0 cellpadding=0>
										<tr>
											<td align=absmiddle>
												<input type=text name=userid size=12 maxlength=16 value="${Session_User.userid }" style="width:120" readonly>
											</td>
											<td>
							...
							<tr>
								...
								<TD BGCOLOR=WHITE>
								<input type=password name=oldpw size=8 maxlength=12 style="width:80">
								</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<TD BGCOLOR=WHITE>
								<c:if test="${Session_User.tel.length() >10}">
								<fmt:formatNumber var="tnum" value="${Session_User.tel}" pattern="000,0000,0000"/>
									<input type=text name=tel size=13 maxlength=20 value="${fn:replace(tnum, ',', '-')}" readonly>
								</c:if>
								<c:if test="${Session_Usertel.length() <10}">
								<fmt:formatNumber var="tnum" value="${Session_User.tel}" pattern="000,000,0000"/>
									<input type=text name=tel size=13 maxlength=20 value="${fn:replace(tnum, ',', '-')}" readonly>
								</c:if>
									<input type=text name=tel size=13 maxlength=20 value="${Session_User.tel}" readonly>
								</td>
							</tr>
							<tr>
								...
								<td bgcolor=WHITE valign=middle>
									<input type="text" name="email" maxlength="30" value="${Session_User.email }" readonly>
								</td>
							</tr>
						</table>
						<table cellpadding=0 cellspacing=0 border=0 width=100%>
